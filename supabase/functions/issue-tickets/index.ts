...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const p = body.purchaser || {}, pay = body.payment || {};
    const quantity = Number(pay.ticket_quantity);

    const required = ["student_number","campus","first_name","last_name","email","course","year_level","section"];
    for (const key of required) if (!String(p[key]||"").trim()) throw new Error(`Missing ${key.replaceAll("_"," ")}`);
    if (!Number.isInteger(quantity) || quantity<1) throw new Error("Invalid ticket quantity");
    if (!Number.isFinite(Number(pay.amount_paid)) || Number(pay.amount_paid)<0) throw new Error("Invalid amount paid");

    const holders = Array.isArray(body.ticket_holders) ? body.ticket_holders : [];
    const purchaserName = [p.first_name,p.middle_name,p.last_name].filter(Boolean).join(" ").replace(/\s+/g," ").trim();
    const tokens = [];
    ...
    const {data:created,error:createError} = await admin.rpc("create_registration_and_tickets",{
      p_registration:{
    ...
    let emailSent = false;
    try {
      await sendTicketEmail(admin,registrationId);
      emailSent = true;
    } catch (emailError) {
      await logEmailFailure(admin,registrationId,p.email,emailError);
    }
